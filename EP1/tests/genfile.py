...
for arq in range(0, nfiles):
	...
	while proc < n:
		t0 = rnd.randrange (0, 201)/10. ## processos comecam nos segundos de 0 a 20,0
		t0 = int(round(t0*10))/10.;
		dt = rnd.randrange (1, 21)/10. ## cada processo demora entre 0,1 e 2,0 segundos
		dt = int(round(dt*10))/10.
		deadline = rnd.randrange (10, 61)/10. * dt + t0
