...
static void *run_process(void *pro){
	...
	tim.tv_sec = (long) runtime;
	tim.tv_nsec = (long) (1000000000.*(runtime - tim.tv_sec));
	
	printf("antes runtime %.1f sec %ld %ld\n", runtime, tim.tv_sec, tim.tv_nsec);
	nanosleep(&tim, &tim2);
	puts("depois");
	...
	if(p->dt <= 0 + 1e-8)
		fprintf(out, "%s %.1f %.1f\n", p->name, running_time(), running_time() - p->start); 
	return NULL;
}
